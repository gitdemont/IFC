...
#' @param corner a character string. where to position text in the image.
#' @param bypass logical to avoid several checking. Default is FALSE.
#' Allowed are "TL", "TR", "BL", "BR", for top-left, top-right, bottom-left, bottom-right, respectively.
#' @details One-lined text will be added so has to be fully contained within image
#' and anchored at desired corner plus x and y offset from it.
#' @return an image with text.
objectAddText <- function(image, text, color, xoff = 0, yoff = 0, corner = "TL", bypass = FALSE) {
  # assert + check
  ...
  if((dt[1] + yoff > di[1]) | dt[2] + xoff > di[2]) {
    txt_msk = cpp_crop(txt_msk, new_height = di[2] - xoff, new_width = di[1] - yoff)
  }
