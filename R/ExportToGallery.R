...
#' @description
#' Exports gallery of IFC objects
...
#' @param export_to used when 'export' is "file" or "base64" to compute respectively filename or base64 id attribute.
...
#' -\%o: with objects (will be collapse with "_", if more than one).\cr
#' A good trick is to use:\cr
#' -"\%d/\%s_gallery_\%o.tiff", when 'export' is "file"\cr
#' -"\%s_gallery_\%o.bmp", when 'export' is "base64".\cr
#' Note that if missing and 'export' is not "file", 'export_to' will be set to "\%s_gallery_\%o.bmp".
...
#' @param ... other arguments to be passed to \code{\link{objectExtract}} with the exception of 'ifd', 'mode'(="rgb") and 'bypass'(=TRUE).
...
#' @export
ExportToGallery <- function(display, offsets, objects, objects_type = "img", layout, 
                            export = c("file", "matrix", "base64")[2],
                            export_to, base64_id = FALSE, base64_att = "", overwrite = FALSE,
                            main = "", add_channels = TRUE, add_ids = 1, add_lines = 2,
                            bg_color = "grey20", dpi = 300, scale = list(),
  ...
  param_extra = names(dots) %in% c("ifd","export","export_to","mode","size","force_width","verbose","bypass","overwrite")
  ...
  if(export != "matrix") {
    ...
    if(any(splitp_obj$channel > 0)) message("'export_to' has %c argument but channel information can't be retrieved with ExportToGallery()")
    export_to = formatn(splitp_obj, splitf_obj, object = paste0(sprintf(paste0("%0",N,".f"),objects), collapse="_"))
    if(export == "file") {
  ...
  tryCatch({
    ...
    col = "white"
    if(objects_type == "img") { 
      if(add_ids > 0) if(any(display$brightfield$channel)) if(channel_id[layout[add_ids]] %in% as.character(which(display$brightfield$channel))) col = "black"
    ...
    ans = lapply(1:length(objects), FUN = function(i) {
      ...
      if(add_ids > 0) img[[add_ids]] = objectAddText(img[[add_ids]], text = id, col, xoff = 1, yoff = 1)
    ...
    # add scale bar
    if(length(scale) != 0) ret = do.call(what = "objectAddScaleBar", args = c(list(image = ret), scale))
    if(export == "file") {
      args = list(filename = export_to, width = zoom*d[2], height = zoom*d[1], units = "px", res = 96)
