...
Rcpp::CharacterVector cpp_type(RObject x) {
  switch( TYPEOF(x) ){
  ...
  default: Rcpp::stop("cpp_type: not supported type in 'x'"); // no support for obj
  }
}
...
bool cpp_allowed (RObject x, RObject alw) {
  bool out = true;
  R_len_t i, j;
  switch( TYPEOF(x) ) {
  case LGLSXP: {
  ...
  case RAWSXP: {
    ...
    break;
  default: Rcpp::stop("cpp_allowed: not supported type in 'x'"); // no support for obj
  }
  return out;
}
